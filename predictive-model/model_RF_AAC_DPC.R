...
cancer_des[is.nan(cancer_des)] <- 0
fungus_des <- t(sapply(fungus, AAC_DPC))
fungus_des[is.nan(fungus_des)] <- 0
bacteria_des <- t(sapply(bacteria, AAC_DPC))
bacteria_des[is.nan(bacteria_des)] <- 0
virus_des <- t(sapply(virus, AAC_DPC))
virus_des[is.nan(virus_des)] <- 0
negative_des <- t(sapply(negative, AAC_DPC))
negative_des[is.nan(negative_des)] <- 0
